...
lazy_static! {
    static ref DEVICES_LONG_REGEX: Regex = Regex::new(r"^(?P<identifier>\S+)\s+(?P<state>\w+) ((usb:(?P<usb1>.*?)|(?P<usb2>\d-\d(\.\d)?)) )?(product:(?P<product>\w+) model:(?P<model>\w+) device:(?P<device>\w+) )?transport_id:(?P<transport_id>\d+)$").expect("cannot build devices long regex");
